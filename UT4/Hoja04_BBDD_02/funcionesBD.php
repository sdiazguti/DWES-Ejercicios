...
function getLibros(){
    ...
    if ($resultado = $pdo->query($consulta)) {
        ...
        while ($libro = $resultado->fetch(PDO::FETCH_OBJ)) {
            $libros[]= array("numeroEjemplar"=>$libro->numero_ejemplar , "titulo"=>$libro->titulo, "anyoEdicion"=>$libro->anyo_edicion, "precio"=>$libro->precio, "fechaAdquisicion"=>$libro->fecha_adquisicion); 
        }
        unset($resultado);
    }
    unset($consulta);
...
function setLibros($titulo,$anyoEdicion,$precio,$anyoAdquisicion){
    ...
    if (true) {//comprobar que la fecha es correcta
        ...
        if ($sentenciaInsertar->exec() !== 0) {
            $pdo->commit();
           return "<div name='alerta'>Datos guardados correctamente</div>";
        } else {
            $pdo->rollback();
           return "<div name='alerta'><strong>ERROR</strong> datos no guardados</div>";
        }
